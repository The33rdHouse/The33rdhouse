...
export default function Portal() {
  ...
  const [step, setStep] = useState(1) // 1: Birth Info, 2: Documents, 3: Journey
  
  // Check if user has already completed portal entry
  useEffect(() => {
    if (user?.portalDocumentsSigned) {
      setStep(3)
    }
  }, [user])
  ...
  // If step 3 (Access Granted/Already Signed), show the Journey
  if (step === 3) {
    return (
      <>
        <SEO {...seoConfig.portal} title="The Portal Journey | The 33rd House" />
        <PortalJourney user={user} />
      </>
    )
  }
