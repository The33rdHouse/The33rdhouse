...
export async function getDb() {
  if (!_db) {
    ...
    if (!databaseUrl) {
      console.warn("[Database] No DATABASE_URL or NETLIFY_DATABASE_URL found");
      return null;
    }
    try {
...
export async function upsertRealmProgress(data: InsertUserProgress) {
  ...
  await db
    ...
    .values(data)
    .onConflictDoUpdate({
      target: [userProgress.userId, userProgress.realmNumber],
      set: {
...
export async function createNotification(data: Omit<InsertNotification, 'userId'> & { userId?: number }) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
...
export async function createChartographyBooking(data: {
  ...
  try {
    const result = await db.insert(chartographyBookings).values({
      ...data,
      birthDate: data.birthDate
    } as any).returning();
    return result[0];
