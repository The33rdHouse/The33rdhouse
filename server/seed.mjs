...
async function seed() {
  ...
  try {
    ...
    for (const gate of gatesData) {
      await db.insert(gates).values(gate).onDuplicateKeyUpdate({
        set: {
          name: gate.name,
          theme: gate.theme,
          shadow: gate.shadow,
    ...
    for (const realm of realmsData) {
      await db.insert(realms).values(realm).onDuplicateKeyUpdate({
        set: {
          name: realm.name,
          gateId: realm.gateId,
    ...
    for (const month of innerCircleMonthsData) {
      await db.insert(innerCircleMonths).values(month).onDuplicateKeyUpdate({
        set: {
          title: month.title,
          gateId: month.gateId,
          theme: month.theme,
    ...
    for (const week of weeksData) {
      await db.insert(innerCircleWeeks).values(week).onDuplicateKeyUpdate({
        set: {
          title: week.title,
