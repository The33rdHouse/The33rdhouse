...
async function createUserPage(databaseId, user, progress) {
  ...
  const createPageCmd = `manus-mcp-cli tool call notion-create-pages --server notion --input '${JSON.stringify({
    ...
    properties: {
      ...
      "Joined": {
        date: { start: new Date(user.createdAt).toISOString().split('T')[0] }
      },
      "Last Active": {
        date: { start: new Date(user.lastSignedIn).toISOString().split('T')[0] }
      }
    }
  })}' 2>&1`;
...
async function main() {
  ...
  try {
    ...
    for (let i = 0; i < users.length; i++) {
      ...
      try {
        // Get user's progress
        const progressRows = await sql`SELECT COUNT(*) as completed FROM user_progress WHERE "userId" = ${user.id} AND completed = true`;
